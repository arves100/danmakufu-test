...
//	ListView_SetImageList(hList, hImageList, LVSIL_SMALL) ;
	...
	viewKey_->AddColumn(220, COL_ACTION, L"Action(動作) [操作]"); //140
	viewKey_->AddColumn(200, COL_KEY_ASSIGN, L"Keyboard(キーボード) [键盘按键]"); //100
...
bool OptionPanel::Initialize(HWND hParent)
{
	...
	viewOption_->AddColumn(480, 0, L"Option(オプション) [选项]"); //292
	viewOption_->SetText(ROW_LOG_WINDOW, 0, L"Show LogWindow(ログウィンドウを表示する) [显示调试窗口]");
	viewOption_->SetText(ROW_LOG_FILE, 0, L"Save LogFile(ログファイルを保存する) [保存调试日志]");
	viewOption_->SetText(ROW_MOUSE_UNVISIBLE, 0, L"Hide Mouse Cursor(マウスカーソルを非表示にする) [隐藏鼠标图标]");
